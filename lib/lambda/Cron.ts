...
if(args.length > 1 && args[1].replace(/\\/g, '/').endsWith('lib/lambda/Cron.ts')) {
  ...
  (async () => {
    ...
    console.log('Next occurrence:', new Cron('0 0 2 * * *', 'America/New_York').nextOccurrence);

    // Get 2AM EST next occurence as UTC
    console.log('Next occurrence:', new Cron('0 0 2 * * *', 'America/New_York').nextOccurrence.toISOString());
